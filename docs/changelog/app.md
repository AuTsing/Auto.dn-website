...
- 修复 WS 远程截图时可能会因锁定画面导致使用了旧图像的问题
...
- 修复 QuickUi 显示时，停止工程会导致工程卡住无法操作的问题
- 修复 QuickUi 获取结果时会获取已移除的历史数据问题

## [3.6.7] - 2026-05-28

- autodncore 内核升级至 v0.23.3

## [3.6.6] - 2026-05-26

- autodncore 内核升级至 v0.23.2

## [3.6.5] - 2026-05-20

- 修复更新应用下载完成后提示没有找到文件的问题

## [3.6.4] - 2026-05-20

- autodncore 内核升级至 v0.23.1
...
- 优化部分悬浮窗被固定在屏幕安全区域显示的问题
- 修复权限成功开启并在应用外撤销权限后，会一直请求权限的问题
- 修复加载工程有新工程时不会回到列表首的问题
- 修复加载工程时有一帧会显示无工程的问题

## [3.4.1] - 2026-04-01

- 优化出现异常时，日志会自动写入
- 修复部分文本信息不能正确显示本地化语言的问题
- 修复上次日志隔天会不显示的问题
...
- autodncore 内核升级至 v0.22.4
- 新增下载扩展支持复制扩展路径
- 新增支持从文件选择器导入工程
- 修复首次启用自动保存日志时提示上次日志不存在的问题
- 修复模拟器在使用ONNX模型时可能会崩溃的问题
- 修复删除工程后仍然聚焦在已删除的工程的问题
- 修复WS服务器运行工程会提示找不到工程的问题
- 修复编辑器中启用输入法时间距不正确的问题
- 修复编辑器关闭首个文本时显示异常的问题
- 修复编辑器Snackbar显示位置不正确的问题
- 修复下载扩展时可能会提示创建文件夹失败的问题
- 修复下载文件会提示文件已存在的问题
- 优化读取工程配置信息的逻辑，工程配置>内部配置>自动生成
- 优化创建配置工程的时候使用文件夹
- 优化移除创建工程时的工程路径配置项，默认创建在外部工程路径
- 优化初始化工程目录的时机

## [3.2.6] - 2026-03-06

- autodncore 内核升级至 v0.22.3
...
- 新增检查更新功能，用于更新应用
- 新增底部导航按钮徽标，以显示未读信息
- 新增克隆仓库功能，支持从远程Git克隆仓库
- 优化使用全新的 Nav3 导航以提升页面导航性能
- 优化添加工程页面、密钥页面、设备信息页面、关于页面使用应用内导航
- 优化工程列表性能
- 优化日志列表性能
- 优化初次进入应用时的引导流程
- 优化错误信息的展示
- 优化悬浮窗服务在后台运行时较容易被关闭的问题
- 修复打包工程功能无法正常运行的问题
- 修复加载上次运行日志在切换应用后可能不正常工作的问题
- 修复执行命令时可能导致内存泄漏的问题
- 修复停止计划任务时会提示错误的问题
...
- 修复读取上次运行日志隔日会失效的问题
- 修复读取上次运行日志在应用没有关闭时会误执行的问题

## [3.1.0] - 2026-01-25

- autodncore 内核升级至 v0.22.2
- 日志界面支持默认读取上次运行时的日志
